...
export class WebhookService {
  ...
  private async deliverWebhook(
    ...
    if (!success && attemptNumber < webhook.retryCount) {
      ...
      this.logger.log(
        `Retrying webhook ${webhook.id} in ${delay}ms (attempt ${attemptNumber + 1})`,
      );
      ...
      this.logger.error(
        `Webhook ${webhook.id} failed after ${attemptNumber} attempts`,
      );
  ...
  getAvailableEvents(): { event: WebhookEvent; description: string }[] {
    return [
      ...
      { event: 'product.deleted', description: 'When a product is deleted' },
      {
        ...
        description: 'When a product reaches low stock threshold',
      },
      {
