...
export class TaxService {
    ...
    async calculateTax(
        ...
        const taxRates = await this.findApplicableTaxRates(organizationId, request.region);
        ...
        for (const item of request.items) {
            ...
            for (const taxRate of taxRates) {
                ...
                const appliesTo =
                    ...
                    (item.category && taxRate.applicableCategories.includes(item.category));
                ...
                if (appliesTo) {
                    ...
                    const existingBreakdown = breakdown.find((b) => b.taxName === taxRate.name);
    ...
    async create(organizationId: string, data: Partial<TaxRate>): Promise<TaxRate> {
