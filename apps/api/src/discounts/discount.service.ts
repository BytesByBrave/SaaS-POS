...
export class DiscountService {
  ...
  async validateAndApply(
    ...
    if (discount.conditions?.dayOfWeek) {
      ...
      if (!discount.conditions.dayOfWeek.includes(dayOfWeek)) {
        return {
          valid: false,
          discountAmount: 0,
          message: 'This discount is not valid today',
        };
      }
    }
    ...
    if (discount.conditions?.timeRange) {
      ...
      if (
        currentTime < discount.conditions.timeRange.start ||
        currentTime > discount.conditions.timeRange.end
      ) {
        return {
          valid: false,
          discountAmount: 0,
          message: 'This discount is not valid at this time',
        };
      }
    }
    ...
    if (applicableItems.length === 0) {
      return {
        valid: false,
        discountAmount: 0,
        message: 'No items in your cart are eligible for this discount',
      };
    }
    ...
    if (
      ...
      applicableSubtotal < discount.minimumOrderAmount
    ) {
      return {
        valid: false,
        discountAmount: 0,
    ...
    const totalItems = applicableItems.reduce(
      (sum, item) => sum + item.quantity,
      0,
    );
    if (
      discount.conditions?.minItems &&
      totalItems < discount.conditions.minItems
    ) {
      return {
        valid: false,
        discountAmount: 0,
