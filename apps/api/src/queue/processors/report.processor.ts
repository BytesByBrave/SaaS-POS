...
export class ReportProcessor {
    ...
    async handleGenerate(job: Job<ReportJobData>) {
        this.logger.log(`Generating ${job.data.reportType} report for org ${job.data.organizationId}`);
        ...
        try {
            ...
            switch (reportType) {
                case 'daily-sales':
                    reportData = await this.generateDailySalesReport(organizationId, dateRange);
                    break;
                case 'inventory':
                    reportData = await this.generateInventoryReport(organizationId);
                    break;
                case 'top-items':
                    reportData = await this.generateTopItemsReport(organizationId, dateRange);
                    break;
                default:
                    throw new Error(`Unknown report type: ${reportType}`);
            }

            this.logger.log(`Report generated successfully for org ${organizationId}`);
            return reportData;
        } catch (error) {
    ...
    async handleDailySummary(job: Job<{ organizationId: string; email: string }>) {
        ...
        this.logger.log(`Daily summary sent to ${email}: ${JSON.stringify(report)}`);
        return report;
    }
    ...
    private async generateTopItemsReport(
        ...
        orders.forEach((order) => {
            order.items?.forEach((item: any) => {
                ...
                itemStats[key].revenue += Number(item.totalPrice || item.price * item.quantity);
            });
        });
